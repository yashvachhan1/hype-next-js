<?php
/**
 * SNIPPET: MODULAR HOME API - NEW ARRIVALS
 * Description: Fetches New Arrival Products for Home Page.
 * Endpoint: /wcs/v1/home/new-arrivals
 */
...
add_action( 'rest_api_init', function() {
    register_rest_route( 'wcs/v1', '/home/new-arrivals', array(
        'methods'  => 'GET',
        'callback' => 'wcs_get_home_new_arrivals',
        'permission_callback' => '__return_true'
    ));
});

function wcs_get_home_new_arrivals() {
    $cache_key = 'wcs_home_new_v2';
    $cached = get_transient($cache_key);
    if ($cached !== false) return new WP_REST_Response($cached, 200);

    global $wpdb;

    // 1. Try Category: 'new-arrivals'
    $sql_cat = "SELECT p.ID, p.post_title, p.post_name, pm_price.meta_value as price, pm_reg.meta_value as regular_price, 
        ...
        WHERE p.post_type = 'product' AND p.post_status = 'publish' AND tt.taxonomy = 'product_cat' AND t.slug = 'new-arrivals'
        GROUP BY p.ID
        ORDER BY p.post_date DESC LIMIT 8";

    $results = $wpdb->get_results( $sql_cat );

    // 2. Fallback: If category empty, use Date Created (True New Arrivals)
    if ( empty($results) ) {
        $sql_date = "SELECT p.ID, p.post_title, p.post_name, pm_price.meta_value as price, pm_reg.meta_value as regular_price, 
            pm_img.meta_value as image_file
        FROM {$wpdb->posts} p
        LEFT JOIN {$wpdb->postmeta} pm_price ON (p.ID = pm_price.post_id AND pm_price.meta_key = '_price')
        LEFT JOIN {$wpdb->postmeta} pm_reg ON (p.ID = pm_reg.post_id AND pm_reg.meta_key = '_regular_price')
        ...
        $results = $wpdb->get_results( $sql_date );
    }
